 ...
 * The class focuses on displaying data and not on collecting the data to display. 
 * The latter is done by the KTableModel which has to be implemented for each 
 * specific case.
 * The table asks the table model for the amount of columns and rows, the sizes 
 * of columns and rows and for the content of the cells which are currently drawn. 
 * Even if the table has a million rows, it won’t get slower because it only 
 * requests those cells it currently draws. Only a bad table model can influence 
 * the drawing speed negatively.
 *
 ...
 * size while the rows are all of the same height except the first row. 
 * Multiple column and row headers are possible. These "fixed" cells will not 
 * be scrolled out of sight. The column and row count always starts in the upper 
 * left corner with 0, independent of the number of column headers or row headers. 
 * 
...
extends Canvas 
{
	 ...
	 * 
	 * 	possible styles:
	 * 	SWT.V_SCROLL - show vertical scrollbar and allow vertical scrolling by arrow keys
	 * 	SWT.H_SCROLL - show horizontal scrollbar and allow horizontal scrolling by arrow keys
	 * 
	...
	{	
		...
		if (m_Model != null) {
			drawBottomSpace(gc);
			drawCells(gc, gc.getClipping(), 
				0, m_Model.getFixedColumnCount(), 
	...
	protected void drawBottomSpace(GC gc) {
		...
		r.y = m_Model.getFirstRowHeight() 
			+ (m_Model.getFixedRowCount() - 1 + m_RowsVisible) * m_Model.getRowHeight() + 1;
		...
		//gc.drawLine(0, m_Model.getFirstRowHeight(), getLastColumnRight(), m_Model.getFirstRowHeight());
		gc.drawLine(0, 0, 0, r.y - 1);
		gc.setForeground(m_Display.getSystemColor(SWT.COLOR_WHITE));
		gc.drawLine(1, r.y - 1, getLastColumnRight() + 1, r.y - 1);
	...
	protected void drawCell(GC gc, int col, int row) {
		...
		m_Model.getCellRenderer(col, row).drawCell(gc, 
			rect, 
			col, 
			row, 
	...
	protected boolean showAsSelected(int col, int row)
	{
		...
		if (m_CellEditor != null)
		{
			if (col == m_CellEditor.m_Col && row == m_CellEditor.m_Row) return false;
		}
	...
	/* gibt die Nummer einer Modellspalte zurück */
	protected int getColumnForResize(int x, int y) {
		if ((y <= 0) || (y >= m_Model.getFirstRowHeight() 
	...
	/* gibt die Nummer einer Zeile der Ansicht(!) zurück */
	...
	/**
	 * Returns the number of the row that is present at position y
	 * or -1, if out of area.
	 * @param y
	 * @return int
	 */
	public int calcRowNum(int y) {
		if (y < m_Model.getFirstRowHeight()) return 0;
	...
	/**
	 * Returns the number of the column that is present at position x
	 * or -1, if out of area.
	 * @param y
	 * @return int
	 */
	...
	public boolean isCellVisible(int col, int row)
	{
		...
		return ((col >= m_LeftColumn 
				&& col < m_LeftColumn + m_ColumnsVisible
				&& row >= m_TopRow
				&& row < m_TopRow + m_RowsVisible)
	...
	/*
	 * Focusses the given Cell.
	 * Assumes that the given cell is in the viewable area.
	 * Does all neccessary redraws.
	 */
	 ...
	 * 
	 * Is also called if user doubleclicks in the resize area
	 * of a resizable column.
	 * 
	 * The optimal width is determined by asking the
	 * CellRenderers for the visible cells of the column
	 * for the optimal with and taking the minimum of the 
	 * results. Note that the optimal width is only determined
	 * for the visible area of the table because otherwise
	 * this could take very long time.
	 * @param column
	 * The column to resize
	 * @return int
	 * The optimal with that was determined or -1, 
	...
	protected void openEditorInFocus() {
	...
	/*
	 * Tries to open KTableCellEditor on the given cell.
	 * If the cell exists, the model is asked for an editor
	 * and if there is one, the table scrolls the cell
	 * into the view and openes the editor on the cell.
	 * @param col
	 * @param row
	 
	public void tryToOpenEditorAt(int col, int row)
	{
		if (col >= 0 && col < m_Model.getColumnCount()
			&& row >= 0 && row < m_Model.getRowCount())
		{
			m_CellEditor = m_Model.getCellEditor(col, row);
			if (m_CellEditor != null) {
				m_FocusCol = col;
				m_FocusRow = row;
				scrollToFocus();
	...
	*/
	 ...
	 * Note that the listener is not called for each cell that the user
	 * selects in one action using Shift+Click. To get all these cells
	 * use the listener and getCellSelecion() or getRowSelection().
	 * 
	 * @param listener
	 */
	 ...
	 * The current selection is cleared when this method is called.
	 * 
	 * @param rowSelectMode
	 * In the "Row Selection Mode", the table always selects a complete row.
	 ...
	 * In the "Multi Select Mode", more than one cell or row can be selected.
	...
	public void clearSelection()
	{
		...
		{
			if (m_Selection.size() < m_RowsFullyVisible * m_ColumnsVisible)
			{
				if (m_RowSelectionMode)
				{
					for (Iterator iter = m_Selection.values().iterator(); iter.hasNext();) {
						int row = ((Integer) iter.next()).intValue();
						if (row >= m_TopRow && row < m_TopRow+m_RowsFullyVisible)
						{
	...
	{
		if (col < m_Model.getColumnCount() && col >= m_Model.getFixedColumnCount()
			&& row < m_Model.getRowCount() && row >= m_Model.getFixedRowCount())
		{
			focusCell(col, row, 0);
			if (scroll)
			{
	...
	public boolean isCellSelected(int col, int row)
	{
		if (!m_MultiSelectMode) 
		{
			if (m_RowSelectionMode) return (row == m_FocusRow);
	 ...
	 * Returns always false if not in Row Selection Mode!
	 * @param row
	 * @return boolean
	 */
	...
	{
		if (!m_RowSelectionMode) return null;
		if (!m_MultiSelectMode)
		{
	 ...
	 * Returns null if in Row Selection Mode.
	 * Returns an array with one or none element if not in Multi Selection Mode.
	 * @return int[]
	 */
	public Point[] getCellSelection()
	{
		...
		return (Point[])m_Selection.values().toArray();
	}
