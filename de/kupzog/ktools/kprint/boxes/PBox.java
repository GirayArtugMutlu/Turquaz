...
public class  PBox {
	// Styles
	/**
	 * Style flag that forces the PBox to be in a new line 
	 * below the previous one
	 * Cannot be used simultaniously to POS_RIGHT.
	 */
	public static final int POS_BELOW = 1;
	/**
	 * Style flag that forces the PBox to be in line 
	 * with the pre vious one. 
	 * Cannot be used simultaniously to POS_BELOW.
	 */
	public static final int POS_RIGHT = 2;
	/**
	 * Style flag that can be used additionally to POS_BELOW/POS_RIGHT 
	 * to tell the PBox that it shoul consume all available
	 * horizontal space on the page.
	 */
	public static final int GRAB = 4;
	/**
	 * Style flag that is mainly used by tables
	 * and forces all PBoxes in a line to have the same
	 * height. All PBoxes in the line should be constructed with
	 * this flag
	 */
	...
	public PBox(PContainer parent, int style)
	{
		this(parent);
		below = true;
		if ((style & POS_RIGHT) > 0) below = false;
		if ((style & POS_BELOW) > 0) below = true;
		if ((style & GRAB) > 0) grabbing = true;
		if ((style & ROW_ALIGN) > 0) rowAlign = true;
	}
	 ...
	 * @param style
	 * Poition: POS_BELOW or POS_RIGHT.
	 * Also GRAB and/or ROW_ALIGN.
	 * @param hWeight
	 * Determines, how much of the existing page width should be occupied.
	 * 1.0 = full page.
	 * -1 = fiexed width, see minCm.
	 * @param minCm
	...
	/*
	 * Sets the forced height. This means that the PBox will have
	 * this height (in pixel) regardless of how high it wants to be.
	 */
	...
	{
		Point originForDrawing = new Point(
			this.origin.x + originOffset.x, 
		...
		if (layoutIsOnPage(page))
		{
			...
			{
				gc.fillRectangle(
					originForDrawing.x+width - boxStyle.getLineWidth(1), 
					originForDrawing.y, 
					boxStyle.getLineWidth(1), 
					height);
			}
			if (boxStyle.hasLine(2))
			{
				gc.fillRectangle(
					originForDrawing.x, 
					originForDrawing.y+ height -  boxStyle.getLineWidth(2), 
					width, 
	...
	/*
	 * Some elements can occupy more than one Page.
	 * Therefore this function tests if the element has
	 * a part on the given page.
	 * @param page
	 * @return boolean
	 */
	...
	/*
	 * Returns the space in y-direction the Element would 
	 * occupy of the rest of the page if told so.
	 * Convention: this method can be called several times for one page,
	...
	/*
	 * Returns true if the box would fit or at least finish
	 * into/within the given space in y-direction. 
	 * Convention: this method can be called several times for one page,
	 * but only until layoutOccupy is called once for this page. 
	 * @param spaceLeft
	 */
	...
	/*
	 * Tells the element to occupy the given space on the page.
	 * Returns the space in y-direction the Element  
	 * occupys of the rest of the page.
	 * Convention: this method is only called once for one page,
	 * and after this call there will be no further
	 * layoutHowMuchWouldYouOccpy-calls for this page.
	 * @param spaceLeft
	 * @return int
	 */
	...
	*/
	...
	/*
	 * Gives the horizontal size of the element.
	 * (has only to work AFTER the layout process,
	 * is used by draw().)
	 */
	public int getWidth()
	{
		if (grabbing) return grabWidth;
		if (hWeight < 0) return pixelX(minCm);
		return Math.max(pixelX(minCm), pixelX(parent.getPossibleWidth()*hWeight));
	}
		
	/*
	 * Gives the vertical size of the element.
	 * Used by all layout* functions.
	 * If multipage functionallity is needed,
	 * this mechanism does no longer work. 
	...
	public int getHeight(int page) {
		if (origin.page == page)
		{ 
			if (rowAlign) return forcedHeight;
			return getHeight(); 
		} 
		return 0;
	}
	...
	public static void setParameters(GC theGC, Device theDevice, Point dpi, int percent)
	{
		...
		pixelPerInch = dpi;
		pixelPerCm = new Point(
			(int)Math.round(dpi.x/2.54), 
			(int)Math.round(dpi.y/2.54)
		);
	}
