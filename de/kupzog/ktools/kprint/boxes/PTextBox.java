...
/**
 * A printable text info that can occupy more than one page. If you are shure that the box will not be bigger than one page, you can use
 * PLittleTextBox to make the layout process quicker.
 * 
		...
		if (layoutIsOnPage(page))
		{
			...
			for (int i = 0; i < part.numOfLines; i++)
			{
				...
				else if (textStyle.textAlign == PTextStyle.ALIGN_RIGHT)
				{
					...
					//System.out.println("'"+(String)textLines.get(part.startLine + i)+"' I="+i+" width = "+textWidth+ "
					// align="+alignPixel);
				}
				gc.drawText((String) textLines.get(part.startLine + i), part.origin.x + alignPixel + originOffset.x
						+ pixelX(textStyle.getMarginLeft()), part.origin.y + originOffset.y + (i * lineHeight), true);
			}
		}
	}
