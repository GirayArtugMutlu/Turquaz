 ...
 * 
 * To print a document an a GC, the folowing steps are
 * neccessary:
 * 
 * (1) create the document's content by creating a 
 * PDocument object and adding children.
 * 
 * (2) setting the GC and its resulution by using the
 * static method PDocument.setParameters
 * 
 * (3) layoutouting the children by a call to
 * PDocument.layout()
 * 
 * (4) drawing the documents pages on a GC by calling
 * PDocument.draw(int page)
 * 
 * (5) disposing the GC and all Font objects by calling
...
extends PContainer
{
	 ...
	 * If you e.g. specify A4 paper size and scale factor 0.712, then the
	 * document will be printed like on A3 but scaled on an A4 sheet. 
	 */
	public PDocument(
		double pageWidth, 
		double pageHeight, 
	...
	public PDocument(String docname) {
		this(
			...
			0,
			0,
			(double) PageSetup.scaling/100,
			docname);
			
		double margin = 2.5;
		if (PageSetup.marginStyle == PageSetup.MARGIN_HUGE) margin = 4;
		if (PageSetup.marginStyle == PageSetup.MARGIN_SMALL) margin = 1;
		margins[0] = margin;
		margins[1] = margin;
		margins[2] = margin;
		margins[3] = margin;
	...
	/**
	 * returns the size of the page area in pixels
	 * given the current dpi setting set by PBox.setParameters.
	 * The page borders are not included.
	 */
	public Point getPageSize(int page)
	{
		Point erg = new Point(
			PBox.pixelX(pageWidth-margins[1]-margins[3]), 
	...
	/**
	* use this method to make all tuning variables
	* of this document and all its children
	* unvalid and so force a recalculation of these
	* variables.
	*/
	...
	/**
	 * Determines all positions of elements and calculates
	...
	public void layout()
	{
		...
		{
			/*
			Rectangle trim = ((Printer) PBox.device).computeTrim(0, 0, PBox.pixelX(pageWidth), PBox.pixelY(pageHeight));
			offsetX = trim.x;
			offsetY = trim.y;
			System.out.println(" Trim: "+trim);
			
			System.out.println("Vorher Page: " + pageHeight + " x " + pageWidth + " cm");
			System.out.println("ClientArea: " + clientArea);
			pageHeight = (double)(clientArea.height/PBox.device.getDPI().y)*2.54;
			pageWidth = (double)(clientArea.width/PBox.device.getDPI().x)*2.54;
			System.out.println("Nachher Page: " + pageHeight + " x " + pageWidth + " cm");
			
			*/
			/*
			Rectangle trim = ((Printer) PBox.device).computeTrim(0, 0, 0, 0);
			System.out.println(" Trim: "+trim);
			System.out.println(" DPI: "+PBox.device.getDPI());
			double[] unprintableMargin = new double[4];
			unprintableMargin[0] = -trim.y;
			unprintableMargin[1] = trim.width + trim.x;
			unprintableMargin[2] = trim.height + trim.y;
			unprintableMargin[3] = -trim.x;
			
			unprintableMargin[0] = (double)(unprintableMargin[0]/PBox.device.getDPI().y)*2.54;
			unprintableMargin[1] = (double)(unprintableMargin[1]/PBox.device.getDPI().x)*2.54;
			unprintableMargin[2] = (double)(unprintableMargin[2]/PBox.device.getDPI().y)*2.54;
			unprintableMargin[3] = (double)(unprintableMargin[3]/PBox.device.getDPI().x)*2.54;
			
			for (int i = 0; i < unprintableMargin.length; i++) {
				System.out.print("Rand " + i + " Vorher: "+margins[i]);
				margins[i] -= unprintableMargin[i];
				if (margins[i] < 0) margins[i] = 0;
				System.out.println(" nachher: "+margins[i]);
			}
			*/
		...
		originOfPage = new Point(
			PBox.pixelX(margins[3])+offsetX, 
			PBox.pixelY(margins[0])+offsetY);
		//System.out.println(" originOfPage: "+originOfPage);
		
		originOnFirstPage = new Point(
			PBox.pixelX(margins[3])+offsetX, 
			PBox.pixelY(margins[0])+getFirstHeader().getHeight()+offsetY);
		//System.out.println(" originOnFirstPage: "+originOnFirstPage);
		
		originOnOtherPages = new Point(
			PBox.pixelX(margins[3])+offsetX, 
			PBox.pixelY(margins[0])+getHeader().getHeight()+offsetY);
		//System.out.println(" originOnOtherPages: "+originOnOtherPages);
		
		originFirstFooter = new Point(
			PBox.pixelX(margins[3])+offsetX, 
			PBox.pixelY(getPageHeight()-margins[2])-getFirstFooter().getHeight()+offsetY); 
		//System.out.println(" originFirstFooter: "+originFirstFooter);
		
		originOtherFooters = new Point(
			PBox.pixelX(margins[3])+offsetX, 
			PBox.pixelY(getPageHeight()-margins[2])-getFooter().getHeight()+offsetY); 
	 ...
	 * call PDocument.layout()
	 * @return int
	 */
	public int getNumOfPages()
	{
		/*if (numOfPages == -1) 
			throw new NullPointerException("Please call layout() first.");*/
	 	return numOfPages;
	}


	/**
	 * Draws the given page number on the GC set by 
	 * PBox.setParameters
	 * Before calling this function, you have to
	 * call PDocument.layout()
	 */
	public void draw(int page)
	{
		...
		if (page == 1)
		{
			if (getHeader(page) != null) getHeader(page).draw(1, originOfPage);
			super.draw(page, originOnFirstPage);
			if (getFooter(page) != null) getFooter(page).draw(1, originFirstFooter);
		}
		else
		{
			if (getHeader(page) != null) getHeader(page).draw(1, originOfPage);
			super.draw(page, originOnOtherPages);
			if (getFooter(page) != null) getFooter(page).draw(1, originOtherFooters);
		}
	...
	/**
	 * If you have already set the first footer,
	 * you can use this method to make all footers
	 * equal to this first footer.
	 */
	...
	/**
	 * If you have already set the first header,
	 * you can use this method to make all headers
	 * equal to this first header.
	 */
