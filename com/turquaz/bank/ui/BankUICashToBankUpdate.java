...
* *************************************
*/
public class BankUICashToBankUpdate extends org.eclipse.swt.widgets.Dialog {
		...
		try {
			...
			dialogShell.setSize(555, 300);
            {
                ...
                toolBar1.setLayoutData(toolBar1LData);
                {
                    ...
                    toolUpdate.setText(Messages.getString("BankUICashToBankUpdate.0")); //$NON-NLS-1$
                ...
                }
                {
                    ...
                    toolDelete.setText(Messages.getString("BankUICashToBankUpdate.2")); //$NON-NLS-1$
                ...
                }
                {
                    ...
                    toolCancel.setText(Messages.getString("BankUICashToBankUpdate.4")); //$NON-NLS-1$
		...
		if(it.hasNext())
		   {
		       ...
		       compCashTrans.getComboCurrencyType().setText(bankTrans.getTurqCurrencyExchangeRate().getTurqCurrencyByExchangeCurrencyId().getCurrenciesAbbreviation());
	...
	public void update(){
	    try{
	        ...
	        BankBLTransactionUpdate.updateCashTransactionBill(transBill,compCashTrans.getTxtBankCard().getTurqBank(),
	                									  compCashTrans.getCurrentPicker().getTurqCashCard(),
	                									 compCashTrans.getCurAmount().getBigDecimalValue(),
	                									 compCashTrans.getDatePick().getDate(),
	                									 compCashTrans.getTxtDefinition().getText().trim(),
	                									 compCashTrans.getTxtDocNo().getText().trim(),
														 compCashTrans.getExchangeRate()
	        		);
	...
	public void delete(){
	    try{
	        ...
	        if(EngUICommon.okToDelete(getParent()))
	        {
	            ...
	            EngUICommon.showMessageBox(getParent(),Messages.getString("BankUICashToBankUpdate.1"),SWT.ICON_INFORMATION);  //$NON-NLS-1$
	            isUpdated = true;
	            dialogShell.close();
	        }
